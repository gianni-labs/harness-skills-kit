...
**Cierre de fase documental.** Toda skill de fase (1–5), al terminar su Fase 3, cierra igual: (1) integra el `preguntas-*.md` y lo archiva en `_archivo/`; (2) actualiza `INDICE.md` con el nuevo estado; (3) **si `Git: respaldo local en gates: sí`**, hace `git commit` **local** acotado a `documentacion/` (sin remoto ni push), con un mensaje que nombre la fase cerrada. Las skills referencian esta regla; no la redefinen.
...
El kit incluye personas especializadas para **revisión adversarial**, disponibles como **prompts de referencia** en `${CLAUDE_PLUGIN_ROOT}/skills/_harness/agentes/` (índice: su `README.md`):
...
**Cómo se usan:** son la grilla de revisión, no necesariamente sub-agentes registrados. Si el entorno las expone como sub-agentes invocables (review/verify/research), úsalas así; si no, se aplica la misma grilla manualmente sobre el diff. **El gate no se salta** en ningún caso. Ambas **respetan los contratos** como fuente de verdad: revisan cumplimiento, no reabren diseño.

> **Pendiente (post-instalación real):** para que Claude Code las registre como sub-agentes invocables de plugin, deberían vivir en `agents/` en la raíz del plugin. Hoy son referencia portable; ver `codex-docs/revision-claude-harness.md` (H-01, C2).
